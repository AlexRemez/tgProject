...
def auth() -> InlineKeyboardMarkup:
    ...
    kb.add(InlineKeyboardButton(text="✖️Отмена✖️", callback_data="AuthCancel"))
    kb.adjust(1)
...
def telephone() -> ReplyKeyboardBuilder:
    ...
    kb.add(KeyboardButton(text="Пропустить➡️"))
    return kb.as_markup(resize_keyboard=True, one_time_keyboard=True, is_persistent=True)


def status() -> InlineKeyboardMarkup:
    kb = InlineKeyboardBuilder()
    kb.add(InlineKeyboardButton(text="⚫️Тренер⚫️", callback_data="status.coach"))
    kb.add(InlineKeyboardButton(text="⚪️Ученик⚪️", callback_data="status.student"))
    kb.adjust(1)
    return kb.as_markup(resize_keyboard=True)
...
def confirm() -> InlineKeyboardMarkup:
    ...
    kb.add(InlineKeyboardButton(text="✖️Что-то не так✖️", callback_data="AuthCancel"))
    kb.adjust(1)
    return kb.as_markup(resize_keyboard=True)
